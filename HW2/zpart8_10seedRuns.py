...
# Define your headers
headers = ["Iteration", "Epoch", "Train Acc", "Val Acc"]

# Open the file in write mode to create it or clear it if it already exists
with open('zpart8_10run.csv', 'w', newline='') as file:
...
for x in range(10):
    ...
    with open('zpart8_10run.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        # This time, you're appending data after the iteration completes
        # Here, you'd calculate or finalize train_acc and val_acc for the whole iteration if needed
        writer.writerow([x+1, "Final", train_acc, val_acc])