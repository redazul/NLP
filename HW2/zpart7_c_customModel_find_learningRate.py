...
with open('training_log.csv', mode='w', newline='') as file:
    ...
    writer.writerow(['Learning Rate', 'Training Accuracy', 'Validation Accuracy'])
    ...
    for x in range(100):
        ...
        optimizer = tf.keras.optimizers.Adam(learning_rate=myLearningRate)
        NUM_EPOCHS = 5
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            print(f"LearningRate {myLearningRate}, Epoch {epoch+1}, Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}, Train Acc: {train_acc:.4f}, Val Acc: {val_acc:.4f}")
        ...
        # At the end of each iteration, write the metrics to the CSV
        with open('zpart7_c_log.csv', mode='a', newline='') as file_append:
            writer_append = csv.writer(file_append)
            writer_append.writerow([myLearningRate, train_acc, val_acc])
