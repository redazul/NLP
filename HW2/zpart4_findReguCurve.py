...
regularization_strength = 0.05  # Starting regularization strength
min_strength = 0.025  # Minimum regularization strength
step_size = 0.00025  # Step size

# Calculate the number of iterations needed
num_iterations = int((regularization_strength - min_strength) / step_size) + 1
print("Print num_iterations:"+str(num_iterations))

accuracies = []  # Store test accuracies for each iteration
...
for i in range(num_iterations):
    ...
    # Train the model
    history = model.fit(X_train_padded, y_train, epochs=70, batch_size=2048, validation_data=(X_test_padded, y_test),verbose=0, shuffle=True)
    ...
    print(f"Iteration {i+1}, Regularization Strength: {regularization_strength:.5f}, Training Accuracy: {train_accuracy}, Test Accuracy: {test_accuracy}")
    
    # Store the accuracies
    accuracies.append(test_accuracy)
    train_accuracies.append(train_accuracy)
    
    # Update the regularization strength for the next iteration
